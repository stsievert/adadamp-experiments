...
from torchvision.datasets import FashionMNIST, CIFAR10, MNIST, ImageNet
...
def main(
    ...
    assert dataset in ["fashionmnist", "cifar10", "synthetic", "mnist", "imagenet"]
    if dataset == "imagenet":
        transform_train = [
    ...
    opt_kwargs = {
        k: args[k] for k in ["initial_batch_size", "max_batch_size", "random_state"]
    }
...
def choose_params_and_run(parameters=None, param_seed=0, damper="sgd", **kwargs):
    ...
    assert all(
        c not in params.keys() and c not in kwargs.keys()
        for c in ["damper", "param_seed"]
    )
